...
├── 💾 数据目录
│   └── data/
│       ├── subscriptions.csv   # 主订阅数据
│       ├── Service.csv         # 服务类型枚举
│       ├── Subscribe.csv       # 订阅周期枚举
│       ├── exchangerate.csv    # 汇率缓存
│       └── history.csv         # 历史记录
│
├── 💻 源代码
│   └── src/
...
│       │   ├── exporter.py     # 报告导出（Excel/CSV/Markdown）
│       │   ├── history.py      # 历史趋势分析
│       │   └── validator.py    # 数据验证
│       │
│       └── components/         # UI 组件包
...
- `src/main.py` - 添加路由
- `pyproject.toml` - 添加依赖（如需要，使用 `uv add <package>`）
...
- 所有源代码 (`src/`)
- 所有文档 (`.md` 文件)
- 配置文件 (`.gitignore`, `.cursorrules`, `pyproject.toml`)
...
- `pyproject.toml` - 添加新依赖时（使用 `uv add`）
- `.cursorrules` - 更新开发规范时
- 文档文件 - 功能变更时
